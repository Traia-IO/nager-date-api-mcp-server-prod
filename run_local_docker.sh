...
# Script to build and run the Nager Date API MCP Server locally in Docker
...
IMAGE_NAME="nager-date-api-mcp-server"
CONTAINER_NAME="nager-date-api-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Nager Date API MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Nager Date API MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
echo
echo -e "${GREEN}✅ Nager Date API MCP Server is running!${NC}"
echo
