#!/usr/bin/env python3
"""
Nager Date API MCP Server - FastMCP with D402 Transport Wrapper
...
logger = logging.getLogger('nager-date-api_mcp')
...
logger.info("="*80)
logger.info(f"Nager Date API MCP Server (FastMCP + D402 Wrapper)")
logger.info(f"API: http://date.nager.at/")
logger.info(f"Payment: {SERVER_ADDRESS}")
logger.info("="*80)

# Create FastMCP server
mcp = FastMCP("Nager Date API MCP Server", host="0.0.0.0")
...
# - NAGER_DATE_API_API_KEY: Server's internal API key for payment mode
#
...
async def determines_whether_today_is_a_public_holiday_in_the_specified_country_optionally_adjusted_by_a_utc_offset(
    ...
    try:
        url = f"http://date.nager.at/api/v3/IsTodayPublicHoliday/{countryCode}"
        params = {
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="nager-date-api-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "nager-date-api-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting Nager Date API MCP Server")
    logger.info("="*80)
